...
export class SponsorComponent implements OnInit {
  ...
  onUpload16(e){
    ...
    const ref = this.storage.ref(filePath);
    const task = this.storage.upload(filePath, file);
    this.uploadPercent16 = task.percentageChanges();
    task.snapshotChanges().pipe( finalize(() => this.urlImage16 = ref.getDownloadURL()) ).subscribe(); 
  }
  ...
  onUpload17(e){
    ...
    const ref = this.storage.ref(filePath);
    const task = this.storage.upload(filePath, file);
    this.uploadPercent17 = task.percentageChanges();
    task.snapshotChanges().pipe( finalize(() => this.urlImage17 = ref.getDownloadURL()) ).subscribe(); 
  }
